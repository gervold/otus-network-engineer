...
Если к соседу применены несколько фильтров в одном направлении, то все фильтры должны разрешить обновление:
...
Особенности:
- Фильтр не может быть применен на соседа, который находится в `peer group`
- Если фильтр был изменен - нужна (иногда) clear команда для применения этих изменений.
- filter-list - фильтрует, prefix-list - и фильтрует и разрешает

Нельзя одновременно использовать `prefix-list` и `distribution-list`, они взаимноисключающие. 
Только один из них может быть применен в каждом направлении (входящем и исходящем) для конкретного соседа.
...
### Настройка фильтров на входящие анонсы в Москве

Предположим, что в Москве не самое новое оборудование.<br> 
В этом случае нам нужна дополнительная фильтрация, на случай внезапного отключения фильтров в провайдерах.

#### Начальные условия

с Ламаса и Киторна временно снимем фильтры (чтобы пропустить тестовый префикс от Киторна – `101.101.101.101`).
Примерно так:

```
R21(config)#router bgp 301
R21(config-router)#no neighbor 30.0.0.15 filter-list 1 out
R21(config-router)# address-family ipv6
R21(config-router-af)#no neighbor 2001:301:15:21::1 filter-list 1 out
R21(config-router-af)#end
```

На R15 проверяем, что префикс появился:
```
R15#show ip bgp
...
Настроим R15 на прием default-маршрута и маршрутов из Питера:
```
ip as-path access-list 2 permit _2042$
ip as-path access-list 2 deny .*

R15(config)#router bgp 1001
R15(config-router)#neighbor 30.0.0.21 filter-list 2 in
```

Проверяем:
```
R15#show ip bgp
BGP table version is 19, local router ID is 30.0.0.15
...
Видим, что `101.101.101.101` - исчез.

Для R14 – аналогично, но фильтры немного другие. Настроим на R14 на прием только default-маршрута:
```
ip prefix-list PL-NO-FULL-TABLE seq 5 permit 0.0.0.0/0
ip prefix-list PL-NO-FULL-TABLE seq 10 deny 0.0.0.0/0 ge 1

router bgp 1001
neighbor 101.0.0.22 prefix-list PL-NO-FULL-TABLE in
```
